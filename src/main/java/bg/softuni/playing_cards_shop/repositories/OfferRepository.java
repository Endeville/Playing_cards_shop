...
public interface OfferRepository extends JpaRepository<OfferEntity, Long> {
    @Query("SELECT o from OfferEntity o " +
            "where (o.status=bg.softuni.playing_cards_shop.models.entities.enums.OfferStatus.APPROVED " +
            "or o.status=bg.softuni.playing_cards_shop.models.entities.enums.OfferStatus.LIMITED)" +
            "and o.deck.title like %:search%")
    List<OfferEntity> findOfferEntityByStatusApprovedOrLimited(@Param(value="search") String search, Sort sort);
}
